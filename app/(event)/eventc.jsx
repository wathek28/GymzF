...
const RegistrationForm = () => {
  ...
  useEffect(() => {
    const checkRegistration = async () => {
      ...
      try {
        const response = await fetch(`http://192.168.1.194:8082/api/events/${eventId}/participation/${userId}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Appel à l'API
      const response = await fetch(`http://192.168.1.194:8082/api/events/${formData.eventId}/participate/${formData.userId}/form`, {
        method: 'POST',
        headers: {
